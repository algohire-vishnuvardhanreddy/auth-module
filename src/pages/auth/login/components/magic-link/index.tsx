...
// Mock API to simulate sending magic link for login
const mockSendMagicLinkLogin = async (
  data: { email: string },
  //@ts-ignore ignore
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  headers: { [key: string]: string }
) => {
  // headers param intentionally unused for now
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      if (data.email) {
        resolve({ success: true, token: "mock-login-token" });
      } else {
...
export default function MagicLinkForm() {
  ...
  useEffect(() => {
    if (isEmailSent) {
      navigate("/login/email-sent", { state: { from: "/magic-link" } });
    }
  }, [isEmailSent, navigate]);

  useEffect(() => {
    if (magicLinkToken) {
      navigate("/"); // Redirect to home, simulating successful login
    }
  ...
  const sendMagicLink = async (token: string) => {
    setIsPending(true);
    try {
      const response = await mockSendMagicLinkLogin(
  ...
  const handleMagicLinkClick = () => {
    form.trigger("email").then((isValid) => {
      ...
      if (window.turnstile) {
        window.turnstile.execute("#turnstile-widget", { action: "magic-link" });
      } else {
                    ...
                    <FormItem className="space-y-1">
                      ...
                      <FormControl>
                        <Input
                          ...
                          className="h-12 rounded-md border-gray-300 dark:border-gray-600 focus:ring-indigo-500 dark:focus:ring-indigo-400 focus:border-indigo-500 dark:focus:border-indigo-400 transition-colors duration-200"
                          {...field}
                          onKeyDown={(e) => {
                            if (e.key === "Enter") {
                              e.preventDefault();
